...
def database():
   ...
   with conn:
      cursor = conn.cursor()
   cursor.execute('SELECT uname FROM admins WHERE uname=?  AND pwd=?',(name1,name2))
...
def signup():
   root.destroy()
   os.system('admin_registration.py')
...
label_1 = Label(root, text="Admin Login Form",bg="#FCCD04",width=33,height=1,font=("bold", 10))
