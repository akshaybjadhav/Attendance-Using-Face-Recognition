...
def storedata():
   ...
   with conn:
      cursor = conn.cursor()
   cursor.execute('SELECT pwd FROM admins WHERE Id=? AND fname=? AND lname=? AND dept=?',(Id1,name1,name2,dept1))
...
label_4 = Label(root, text="Department",bg="#123C69",fg="#FFFFFF",width=20,font=("bold", 10))
label_4.place(x=80,y=180)

entry_4 = Entry(root,textvar=dept)
entry_4.place(x=240,y=180)

Button(root, text='Submit',width=20,bg='#FCCD04',fg='black',command=storedata).place(x=170,y=210)
