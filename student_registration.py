...
def validate():
   ...
   unm=uname.get()
   cursor.execute('Select uname from students where uname=?', (unm,))
...
def storedata():
   ...
   if(gender1==1):
      ...
      gender='female'
   cursor.execute('CREATE TABLE IF NOT EXISTS students(Id TEXT,fname TEXT,lname TEXT,contact TEXT,email TEXT,dept TEXT,gender TEXT,address TEXT,uname TEXT,pwd TEXT)')
   cursor.execute('INSERT INTO students (Id,fname,lname,contact,email,dept,gender,address,uname,pwd) VALUES(?,?,?,?,?,?,?,?,?,?)',(Id1,name1,name2,contact1,email1,dept1,gender,address1,uname1,pwd1))
   conn.commit()
   conn.close()
   root.destroy()
...
label_1 = Label(root, text="Student SignUp Form",bg="#FCCD04",width=33,height=1,font=("bold", 10))
